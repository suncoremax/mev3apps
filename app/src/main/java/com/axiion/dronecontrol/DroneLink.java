 ...
 *
 * That "reply" signal is driven by a dedicated CRTP_PORT_LINK echo ping
 * (see {@link #sendPing()} / {@link CrtpPacket#linkEchoPing}) sent at
 * PING_HZ, on top of the SETPOINT stream. The firmware guarantees a reply
 * to that specific packet (crtpservice.c bounces it straight back); relying
 * on setpoint packets alone for "is it alive" is a dead end because the
 * firmware doesn't talk back on its own for those, and its debug console is
 * UART-only unless DEBUG_PRINT_ON_CONSOLE is set at build time.
 */
final class DroneLink {
    ...
    private static final long PING_PERIOD_MS = 500L; // 2 Hz link-echo heartbeat, separate from the setpoint stream
    ...
    void start(String host, int udpPort) throws IOException {
        ...
        // The firmware's crtp_commander_rpyt.c has a `thrustLocked` safety
        // latch: it force-zeros thrust until it has seen one genuine
        // thrust==0 packet, and — importantly — it stays locked FOREVER if
        // that never happens, even if you lower the stick afterward. Because
        // the throttle stick holds its position (autoCenterY=false) and
        // setSetpoint() can be called from the UI thread at any time, there
        // was a race where the very first packet the drone ever saw after
        // connecting could already be non-zero (e.g. the pilot's thumb is
        // still resting on the throttle stick while the other hand taps
        // ARM) — permanently locking thrust with no way to recover short of
        // reconnecting. Close that race deterministically: force our local
        // setpoint to zero and physically put a few explicit zero-thrust
        // packets on the wire *before* the live 60 Hz loop (which reads
        // whatever the UI has set) ever starts.
        ...
        for (int i = 0; i < 5; i++) {
            try {
                ...
                // best-effort; the regular send loop below will keep trying
            }
            try {
                Thread.sleep(20);
        ...
        pingTask = sendExecutor.scheduleAtFixedRate(this::sendPing, 0, PING_PERIOD_MS, TimeUnit.MILLISECONDS);
    }
    ...
    /** Sends the CRTP_PORT_LINK echo ping the firmware always bounces back — see class javadoc. */
    private void sendPing() {
        DatagramSocket s = socket;
        InetAddress a = address;
        if (s == null || a == null || s.isClosed()) return;

        byte[] bytes = CrtpPacket.linkEchoPing((byte) (pingCounter.getAndIncrement() & 0xFF));
        try {
            s.send(new DatagramPacket(bytes, bytes.length, a, port));
        } catch (IOException e) {
            if (listener != null) listener.onSendError(e.getMessage());
        }
    }
