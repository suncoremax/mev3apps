...
public class MainActivity extends Activity {
    ...
    // Android apps can't silently flip the system location toggle for
    // privacy/security reasons, but we can (and should) show the native
    // "Turn on device location" one-tap dialog automatically — exactly
    // what Maps/Uber/etc. do — instead of leaving the user to dig through
    // Settings themselves. This only shows the dialog when GPS is off;
    ...
    // Instead of jumping straight to WebView.goBack() (which does nothing
    // and exits the app immediately on a single-page app that never
    // changes browser history), we first ask the web app itself whether
    // it wants to handle the back press — e.g. close an open modal, or
    // switch back to the previously opened section — exactly like a
    // native app's back stack. Only if the web app says "nothing to do"
    // do we fall back to WebView history, then finally a
    // press-again-to-exit confirmation.
    @Override
